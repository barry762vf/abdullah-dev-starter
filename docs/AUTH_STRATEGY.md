...
Phase 7 (ADR 014) layers these limits: shared per-IP limits at the proxy/edge (Nginx `limit_req` or Cloudflare WAF rules), a PostgreSQL-backed per-account throttle shared by every worker and instance (10 failed logins within 15 minutes since the last success returns 429 before password hashing), and the original in-process limiter as local defence in depth. The client IP used by all of them comes from `CLIENT_IP_SOURCE`: the socket peer, or `X-Edge-Client-IP` from the authenticated same-origin proxy; `X-Forwarded-For` is never trusted and Uvicorn always runs with `--no-proxy-headers`. See `docs/DEPLOYMENT_STRATEGY.md` §2–3.
